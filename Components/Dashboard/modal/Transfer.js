'use client'
import styled from "styled-components";
import { FaWallet } from "react-icons/fa";
import { useEffect, useState } from "react";
import imageUrlBuilder from "@sanity/image-url";
import { client } from "../../../lib/sanity";
import {
  useTokenBalance,
  useContract,
  useContractWrite,
} from "@thirdweb-dev/react";

const Transfer = ({
  setAction,
  thirdWebTokens,
  selectedToken,
  walletAddress,
  exportStorageContent, // Function to export storage content
}) => {
  const [amount, setAmount] = useState("");
  const [recipient, setRecipient] = useState("");
  const [builder] = useState(imageUrlBuilder(client));
  const [activeTwToken, setActiveTwToken] = useState();
  const [imageUrl, setImageUrl] = useState(null);
  const usedToken = selectedToken.contractAddress;
  const { contract: stakeTokenContract } = useContract(usedToken);
  const { data: tokenBalance, isLoading } = useTokenBalance(
    stakeTokenContract,
    walletAddress
  );
  const { mutateAsync: transfer } = useContractWrite(
    stakeTokenContract,
    "transfer"
  );

  useEffect(() => {
    thirdWebTokens.forEach((token) => {
      if (token === usedToken) {
        setActiveTwToken(token);
      }
    });
  }, [thirdWebTokens, selectedToken.contractAddress]);

  useEffect(() => {
    if (selectedToken.logo) {
      const url = builder.image(selectedToken.logo.asset._ref).url();
      setImageUrl(url);
    }
  }, [selectedToken, builder]);

  const sendCrypto = async () => {
    if (activeTwToken && amount && recipient) {
      setAction("transferring");
  
      try {
        // Make the transfer call
        const result = await transfer({
          args: [recipient, amount.toString().concat("000000000000000000")],
        });
  
        // Log the result of the transfer function
  
        // Save transaction details to localStorage
        saveTransactionToLocal({
          from: result.receipt.from,
          to: result.receipt.to,
          transactionHash: result.receipt.transactionHash, // Add transactionHash
          blockNumber: result.receipt.blockNumber, // Add blockNumber
        });
  
        // Update the UI or perform actions based on the result
        setAction("transferred");
  
        // Export the storage content
        exportStorageContent(getStoredTransactions());
      } catch (err) {
        console.error("Contract call failure:", err);
        setAction("failed");
      }
    } else {
      console.log("Missing Data");
    }
  };

  const saveTransactionToLocal = (transaction) => {
    let storedTransactions = JSON.parse(localStorage.getItem("transactions")) || [];
    storedTransactions.push(transaction);
    localStorage.setItem("transactions", JSON.stringify(storedTransactions));
  };
  

  const getStoredTransactions = () => {
    return JSON.parse(localStorage.getItem("transactions")) || [];
  };

  return (
    <Wrapper>
      <Amount>
        <FlexInputContainer>
          <FlexInput
            placeholder="0"
            type="number"
            value={amount}
            onChange={(e) => setAmount(e.target.value)}
          />
          <span>{selectedToken.symbol}</span>
        </FlexInputContainer>
        <Warning style={{ color: amount && "#0a0b0d" }}>
          Amount is a required field
        </Warning>
      </Amount>
      <TransferForm>
        <Row>
          <FieldName>To</FieldName>
          <div className="mx-2"></div>
          <Icon>
            <FaWallet />
          </Icon>
          <Recipient
            placeholder="Address"
            value={recipient}
            onChange={(e) => setRecipient(e.target.value)}
          />
        </Row>
        <Divider />
        <Row>
          <FieldName>Pay with</FieldName>
          <CoinSelectList onClick={() => setAction("select")}>
            <Icon>
              <img src={imageUrl} alt={selectedToken.name} />
            </Icon>
            <CoinName className="dark:text-white text-black">{selectedToken.name}</CoinName>
          </CoinSelectList>
        </Row>
      </TransferForm>
      <Row>
        <Continue onClick={sendCrypto}>Continue</Continue>
      </Row>
      <Row>
        <BalanceTitle>{selectedToken.symbol} Balance</BalanceTitle>
        <Balance>
          {isLoading ? (
            <div>Loading...</div>
          ) : (
            <div>
              {tokenBalance.displayValue} {selectedToken.symbol}
            </div>
          )}
        </Balance>
      </Row>
    </Wrapper>
  );
};

export default Transfer;

const Wrapper = styled.div`
  display: flex;
  flex-direction: column;
  height: 100%;
  width: 100%;
  flex: 1;
`;

const TransferForm = styled.div`
  border: 1px solid #282b2f;
  border-radius: 0.4rem;
`;

const Amount = styled.div`
  flex: 1;
  display: flex;
  flex-direction: column;
`;

const FlexInputContainer = styled.div`
  flex: 1;
  display: flex;
  align-items: flex-end;

  & > span {
    font-size: 2rem;
    margin-bottom: 0.5rem;
    color: #9747ff;
  }
`;

const FlexInput = styled.input`
  border: none;
  background: none;
  outline: none;
  color: black;
  text-wrap: wrap;
  text-align: right;
  max-width: 45%;
  margin-right: 1rem;
  font-size: 3rem;
  color: #9747ff;

  &::-webkit-outer-spin-button,
  ::-webkit-inner-spin-button {
    -webkit-appearance: none;
  }
`;

const Warning = styled.div`
  /* TRouBLe */
  padding: 1rem 0 2rem 0;
  text-align: center;
  color: #8a919e;
  font-size: 0.8rem;
`;

const Divider = styled.div`
  border-bottom: 1px solid #282b2f;
`;

const Row = styled.div`
  display: flex;
  align-items: center;
  justify-content: space-between;
  color: #8a919e;
  padding: 1rem 0;
  font-size: 1.2rem;
`;
const FieldName = styled.div`
  flex: 0.5;
  padding-left: 2rem;
`;

const Icon = styled.div`
  margin-right: 1rem;
  height: 1.8rem;
  width: 1.8rem;
  border-radius: 50%;
  overflow: hidden;
  display: grid;
  place-items: center;

  & > img {
    // margin: -0.5rem 1rem; 
    height: 100%;
    width: 100%;
    object-fit: cover;
  }
`;

const Recipient = styled.input`
  flex: 1;
  border: none;
  background: none;
  outline: none;
  color: black;
  font-size: 1.2rem;
  text-wrap: wrap;
  margin-right: 0.5rem;
`;

const CoinSelectList = styled.div`
  display: flex;
  flex: 1.3;
  height: 100%;

  &:hover {
    cursor: pointer;
  }
`;

const CoinName = styled.div`
  flex: 1;
  border: none;
  background: none;
  outline: none;
  color: black;
  font-size: 1.2rem;
  text-wrap: wrap;
  margin-right: 0.5rem;
`;

const Continue = styled.button`
  color: white;
  width: 100%;
  background-color: #232a42;
  padding: 1rem;
  text-align: center;
  border-radius: 0.4rem;
  font-size: 1.2rem;

  &:hover {
    cursor: pointer;
    background-color: #5e1db3;
    transform: scale(1.05);
    transition: background-color 0.4s ease, transform 0.4s ease;
  }
`;

const BalanceTitle = styled.div``;

const Balance = styled.div``;
