...
import { SavedContext } from "../../../app/Market/context/Crypto/SavedContext";
import { StockContext } from "../../../app/Market/context/Stock/stockContext";
// import dynamic from "next/dynamic";

// const showCryptoDetail = dynamic(() => import ("../app/CryptoDetail/page"), {ssr:false})
...
const SaveBtn = ({ data }) => {
  ...
  return (
    <button
      className="outline-0 border-0 bg-none cursor-pointer pl-3 hover:scale-110 transition-all transition-ease"
...
const TableComponent = () => {
  let { stockData, currency } = useContext(StockContext);

  return (
    <>
      <div
        style={{ position: "relative", zIndex: "21" }}
        className="flex flex-col mt-9 border  bg-white border-gray-100 rounded"
      >
        {stockData ? (
          <table className="w-full table-auto relative z-21">
            {/* remove text-left */}
            <thead
              style={{
            ...
            >
              <tr>
                {/* remove pl-5 */}
                <th className="pl-5 py-1">Assets</th>
                <th className="py-1 pl-2 text-left md:table-cell hidden">
                  Name
                </th>
                <th className="py-1 lg:table-cell hidden">Total Volume</th>
                <th className="py-1 sm:table-cell hidden text-end md:text-center">
                  Market Cap
                </th>
                <th className="py-1 md:table-cell hidden">P/E</th>
                <th className="py-1 text-right md:text-center text-base">Price</th>
                <th className="py-1 lg:table-cell hidden">Price Change</th>
                <th className="py-1 lg:table-cell hidden">EPS Growth</th>
                <th className="py-1 lg:table-cell hidden">Div Yeild</th>
                <th className="py-1 pl-4 xl:table-cell hidden text-left">
                  Sector
                </th>
              </tr>
            </thead>
            <tbody>
              {stockData.map((data) => {
                let EPS = data.EPS_dil_growth > 0;
                let change = data.change > 0;
                let div_yield = data.div_yield > 0;
                return (
                  <tr
                    // hover:scale-[1.1] hover:shadow-2xl
                    // bg-[#f0f3fa] hover:shadow-inner
                    // id === company
                    key={data.company}
                    // add text-center
                    style={{ fontSize: '1rem', border: '1px solid #e2e8f0', lineHeight: '1.5rem' }}
                    className=" text-center text-base text-black border-b border-gray-100 hover:bg-gray-50 last:border-b-0 cursor-default transition-all transition-ease hover:scale-[1.1] hover:shadow-2xl"
                  >
                    <td className="py-4 flex items-center uppercase">
                      <SaveBtn data={data} />
                      {data.stock_image ? (
                        <Link
                          href={`Stock/${data.stock_name}`}
                          className="cursor-pointer text-black hover:text-[#8018f7] transition-all transition-ease"
                        >
                          <img
                            className="w-[1.2rem] h-[1.2rem] mx-1.5 hover:size-7 transition-all transition-ease rounded-full"
                        ...
                        <Link
                          href={`Stock/${data.stock_name}`}
                          className="cursor-pointer text-black hover:text-[#8018f7] hover:underline transition-all transition-ease"
                        >
                          <span className="w-[1.2rem] h-[1.2rem] mx-1.5 flex items-center justify-center rounded-full bg-[#edeff2] text-black">
                            {data.stock_name.charAt(0)}
                          </span>
                        </Link>
                      )}
                    ...
                    <td className="py-4">
                      <Link
                        href={`Stock/${data.stock_name}`}
                        className="pl-2 cursor-pointer text-left text-black hover:text-[#8018f7] transition-all transition-ease md:table-cell hidden"
                      >
                        {/* name=== company */}
                        {data.company}
                      </Link>
                    ...
                    <td className="py-4 lg:table-cell hidden">{data.volume}</td>
                    <td className="py-4 sm:table-cell hidden text-left md:text-center">
                      {data.market_cap}
                    </td>
                    <td className="py-4 md:table-cell hidden">
                      <span>{data.price_to_earn}</span>
                    </td>
                    <td className="py-4 text-left md:text-center">
                    ...
                    <td
                      ...
                      className={"py-4  lg:table-cell hidden"}
                    >
                      {Number(data.EPS_dil_growth).toFixed(2)}%
                    </td>
                    <td
                      style={{
                        color: div_yield
                      ...
                      className={"py-4 lg:table-cell hidden"}
                    >
                      {Number(data.div_yield).toFixed(2)}%
                    </td>
                    <td className="pl-4 xl:table-cell hidden text-left">
                      {/* name=== company */}
                      {data.sector}
                    </td>
                  </tr>
          ...
          <table>
              ...
              <tr>
                <th className="py-1">
                  <Skeleton
                    ...
                    width={100}
                    height={13}
                    className="ml-2 my-1 h-14 w-12 rounded-md"
                  />
                </th>
                ...
                <th className="py-1 ">
                  <Skeleton
                    ...
                    width={90}
                    height={13}
                    className="h-14 w-13  rounded-md"
                  />
                </th>
                <th className="py-1 md:table-cell hidden">
                  <Skeleton
                    ...
                    width={150}
                    height={13}
                    className="h-14 w-15 rounded-md"
                  />
                </th>
                ...
                <th className="py-1 lg:table-cell hidden">
                  <Skeleton variant="rect" className="h-14 w-10 rounded-md" />
                </th>
                <th className="py-1 lg:table-cell hidden">
                  <Skeleton variant="rect" className="h-14 w-10 rounded-md" />
                </th>
            ...
            <tbody>
              {[...Array(10)].map((_, index) => (
                <tr
                  //
                  className="text-center text-base border-b border-gray-100 hover:bg-gray-50 last:border-b-0"
                  key={index}
                >
                  <th className="py-1">
      ...
      <div className="flex items-center justify-end mt-4 capitalize h-[2rem]">
        <Pagination />
      </div>
    </>
  );
